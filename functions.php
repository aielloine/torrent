...
class Torrent
{
    ...
    public function gen_infos($title){
      ...
      // seed_leech
      $this->seed_leech = array_pop($explode_title);
      // original_title
      ...
      $full_titre = "";
      $TMDB = new TMDB();
      foreach ($explode_title as $number => $mot) {
        $full_titre .= $mot." ";
        $json_info_movi = $TMDB->search($full_titre);

        if ($json_info_movi->total_results != 0) {
          // vrai_oeuvre
          unset($explode_title[$number]);
         $this->vrai_oeuvre = $json_info_movi->results[0];
       }else {
         break ;
       }
      }

      $languages = fk_taxonomies("Video language")->id();
      $languages = fk_terms_by_taxonomy($languages);
      $resolution = fk_taxonomies("Video resolution")->id();
      $resolution = fk_terms_by_taxonomy($resolution);
      $audio = fk_taxonomies("Video audio")->id();
      $audio = fk_terms_by_taxonomy($audio);

      // recherche des métadonnées
      foreach ($explode_title as $mot) {
        if (in_array($mot, $resolution)) {
          $this->resolution[] = $mot;
        }
        if (in_array($mot, $audio)) {
          $this->audio[] = $mot;
        }
        if (in_array($mot, $languages)) {
          $this->language[] = $mot;
        }
      }


      $tvdb_mg_src = "https://image.tmdb.org/t/p/w200";
      $this->type = $this->vrai_oeuvre->media_type;
      switch ($this->type) {
        case 'movie':
          $this->image = $tvdb_mg_src.$this->vrai_oeuvre->poster_path;
          $this->name = $this->vrai_oeuvre->title;
          $this->date =  $this->vrai_oeuvre->release_date;
          break;
        case 'person':
          $this->image = $tvdb_mg_src.$this->vrai_oeuvre->profile_path;
          $this->name = $this->vrai_oeuvre->name;
          $this->date = $this->vrai_oeuvre->birthday;
          break;
        case 'tv':
          $this->image = $tvdb_mg_src.$this->vrai_oeuvre->poster_path;
          $this->name = $this->vrai_oeuvre->name;
          $this->date =  $this->vrai_oeuvre->first_air_date;
          break;
      }
    ...
    //   foreach ($explode_title as $number => $mot) {
    //     $full_titre .= $mot." ";
    //     $multiSearchResults = $tmdb->multiSearch($full_titre);
    //     if (sizeof($multiSearchResults) != 0 && sizeof($multiSearchResults[array_key_first($multiSearchResults)]) != 0) {
    //       // vrai_oeuvre
    //       unset($explode_title[$number]);
    //      $this->vrai_oeuvre = $multiSearchResults;
    //    }else {
    //      break ;
    //    }
    //   }
    //   $languages = array("multi", "vf2", "vff", "vf", "vfi", "vfq", "vo", "vostfr", "vosten", "truefrench", "french", "eng" );
    //   $resolution = array("dvdrip", "dvd-rip", "bdrip", "bd-rip", "480p", "1280x720",  "720p", "1920x1080", "1080p", "3840x2160", "2160p");
    //   $audio = array("aac", "ac3", "mp3", "flac", "dolby",  "truehd", "atmos", "dts", "dts-hd", "dts-hdma", "DDP.5.1");
    //
    //   // recherche des métadonnées
    //   foreach ($explode_title as $mot) {
    //     if (in_array($mot, $resolution)) {
    ...
    //     }
    //   }
    //   $media = array_key_first($this->vrai_oeuvre);
    //   $this->vrai_oeuvre = $this->vrai_oeuvre[$media][0];
    //   $tvdb_mg_src = "https://image.tmdb.org/t/p/w200";
    //   $this->type = $media;
    //   switch ($this->type) {
    //     case 'movie':
    //       $this->image = $tvdb_mg_src.$this->vrai_oeuvre->getPoster();
    //       $this->name = $this->vrai_oeuvre->getTitle();
    //       $this->date =  $this->vrai_oeuvre->get('release_date');
    //       break;
    //     case 'person':
    //       $this->image = $tvdb_mg_src.$this->vrai_oeuvre->getProfile();
    //       $this->name = $this->vrai_oeuvre->name;
    //       $this->date = $this->vrai_oeuvre->birthday;
    //       break;
    //     case 'tv':
    //       $this->image = $tvdb_mg_src.$this->vrai_oeuvre->getPoster();
    //       $this->name = $this->vrai_oeuvre->getName();
    //       $this->date =  $this->vrai_oeuvre->first_air_date;
    //       break;
    //   }
    //
...
//Beautification de la date du film
    function beautify_date(){
          $mois = ["Janvier","Février","Mars","Avril","Mai","Juin","Juillet","Août","Septembre","Octobre","Novembre","Décembre"];
          $explode_date = explode("-",$this->date);
          return $explode_date[2]." ".$mois[$explode_date[1]-1]." ".$explode_date[0];
    }
